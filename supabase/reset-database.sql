...
-- =============================================
-- This script will completely reset your database and recreate it with the perfect schema

-- STEP 1: DROP ALL EXISTING TABLES AND TYPES
-- Drop tables in correct order to avoid foreign key conflicts
DROP TABLE IF EXISTS customer_realtime_trigger CASCADE;
DROP TABLE IF EXISTS sales_realtime_trigger CASCADE;
...
DROP TRIGGER IF EXISTS update_stores_updated_at ON stores CASCADE;
DROP TRIGGER IF EXISTS update_floors_updated_at ON floors CASCADE;
...
DROP TRIGGER IF EXISTS update_interactions_updated_at ON interactions CASCADE;
DROP TRIGGER IF EXISTS update_follow_ups_updated_at ON follow_ups CASCADE;
DROP TRIGGER IF EXISTS update_sales_updated_at ON sales CASCADE;
DROP TRIGGER IF EXISTS update_approval_workflows_updated_at ON approval_workflows CASCADE;
DROP TRIGGER IF EXISTS update_escalations_updated_at ON escalations CASCADE;
DROP TRIGGER IF EXISTS sales_realtime_trigger ON sales CASCADE;
DROP TRIGGER IF EXISTS customer_realtime_trigger ON customers CASCADE;
...
DROP TYPE IF EXISTS follow_up_type CASCADE;
...
-- Drop all functions
DROP FUNCTION IF EXISTS generate_sku(TEXT, UUID) CASCADE;
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS notify_sales_update() CASCADE;
DROP FUNCTION IF EXISTS notify_customer_update() CASCADE;
...
-- STEP 10: ENABLE REAL-TIME FOR ALL TABLES
...
-- STEP 12: CREATE REAL-TIME FUNCTIONS
...
-- STEP 13: CREATE REAL-TIME TRIGGERS
...
/*
-- Sample store
INSERT INTO stores (name, address, city, state, pincode, phone, email, website, description)
VALUES (
  'Sarkar Jewellery Store',
  '123 Main Street, Downtown',
  'Mumbai',
  'Maharashtra',
  '400001',
  '+91-9876543210',
  'info@sarkarjewellery.com',
  'https://sarkarjewellery.com',
  'Premium jewellery store offering the finest collection of gold, silver, and diamond jewellery.'
);

-- Sample floor
INSERT INTO floors (name, number, description, store_id)
SELECT 'Ground Floor', 1, 'Main showroom and reception area', id FROM stores WHERE email = 'info@sarkarjewellery.com';

-- Sample user
INSERT INTO users (name, email, role, store_id, floor_id)
SELECT 
  'Admin User',
  'admin@sarkarjewellery.com',
  'BUSINESS_ADMIN',
  s.id,
  f.id
FROM stores s
JOIN floors f ON f.store_id = s.id
WHERE s.email = 'info@sarkarjewellery.com' AND f.number = 1;
*/

-- STEP 15: VERIFICATION
SELECT 'Database reset completed successfully!' as status;
SELECT COUNT(*) as total_tables FROM information_schema.tables WHERE table_schema = 'public';
SELECT COUNT(*) as total_enums FROM pg_type WHERE typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public') AND typtype = 'e'; 